...
class HomeFrame(BaseClass):
    ...
    def __init__(self, *args, **kwargs):      
        ...
        self.root_btn = Button(self, text="Continue", command=lambda: self.change_frame(PlaylistFrame), padx=50, pady=5, fg="White", bg=self.default_button_color)
...
class PlaylistFrame(BaseClass):
    ...
    def __init__(self, *args, **kwargs):         
        ...
        for idx, playlist in enumerate(playlists_list, start=2):
            r_button = Radiobutton(self, text=playlist[0], variable=self.radio_item, value=playlist[0], padx=50, pady=5, command=lambda: playlist_label.config(text=self.radio_item.get()), fg="Black", bg=self.bg_color)
        ...
        # continue button.
        continue_button = Button(self, text="Continue", padx=20, pady=5, fg="White", bg=self.default_button_color, command=self.continue_with_selection)
        ...
        # back button.
        back_button = Button(self, text="Back", padx=20, command=lambda: self.change_frame(HomeFrame))
    ...
    def __init__(self, playlist_title, sort_name = None, **kwargs):    
        ...
        for sound in playlist:
            if c == 5: 
                r += 1
                c = 0
            sound_button = Radiobutton(self, text=sound[0], variable=self.radio_item, value=sound[0], command=lambda: sound_label.config(text=self.radio_item.get()), fg="Black", bg=self.bg_color, width=20)
        ...
        # play button.    
        play_btn = Button(self, text="Play", command=lambda: self.saturn.play(os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + "/sounds/" + self.radio_item.get() + ".mp3"), padx=50, pady = 5, bg=self.default_button_color, fg="white")
        play_btn.grid(row=r, column=1, pady=20, padx=10)

        # edit button.
        play_edit = Button(self, text="Edit", padx=50, pady = 5, command=self.continue_with_selection, bg=self.default_button_color, fg="white")
        ...
        ok_button = Button(self, text="Go", padx=30, pady=5, command=lambda: self.reload_frame(SoundFrame, playlist_title, self.dropdown_item.get()), bg="green", fg="White")
        ok_button.grid(row=r+4, column=2, pady=20, padx=10)
        
        # back button.
        back_button = Button(self, text="Back", padx=20, command=lambda: self.change_frame(PlaylistFrame))
...
class EditFrame(BaseClass):
    ...
    def __init__(self, sound_title, **kwargs):          
        ...
        # [Sound] title
        Label(self, text=self.sound_title, font=(self.font_title, 20), padx=50, pady=5, fg="Black", bg=self.bg_color).grid(pady=10, row=0, column=2, sticky=EW)
        
        # working in the 0th column: adding ALL edit features.
        Label(self, text="Speed", fg="Black", bg=self.bg_color, font=("Courier New", 10)).grid(row=1, column=0, padx=50, pady=15, sticky=W)
        Label(self, text="Pitch", fg="Black", bg=self.bg_color, font=("Courier New", 10)).grid(row=2, column=0, padx=50, pady=15, sticky=W)
        Label(self, text="Reverse", fg="Black", bg=self.bg_color, font=("Courier New", 10)).grid(row=3, column=0, padx=50, pady=15, sticky=W)
        Label(self, text="Overlap", fg="Black", bg=self.bg_color, font=("Courier New", 10)).grid(row=4, column=0, padx=50, pady=15, sticky=W)
        Label(self, text="Sequential", fg="Black", bg=self.bg_color, font=("Courier New", 10)).grid(row=5, column=0, padx=50, pady=15, sticky=W)
        Label(self, text="Random Insert", fg="Black", bg=self.bg_color, font=("Courier New", 10)).grid(row=6, column=0, padx=50, pady=15, sticky=W)
        ...
        speed_up_button = Button(self, text="Up", fg="Black", bg=self.inactive_color, padx=10, pady=5, command=lambda: adjust_speed_label(True))
        speed_up_button.grid(row=1, column=1, sticky=W)
        speed_down_button = Button(self, text="Down", fg="Black", bg=self.inactive_color, padx=10, pady=5, command=lambda: adjust_speed_label(False))
        ...
        pitch_up_button = Button(self, text="Up", fg="Black", bg=self.inactive_color, padx=10, pady=5)
        pitch_up_button.grid(row=2, column=1, sticky=W)
        pitch_down_button = Button(self, text="Down", fg="Black", bg=self.inactive_color, padx=10, pady=5)
        pitch_down_button.grid(row=2, column=2, sticky=W)
        # pitch Label.
        Label(self, text=pitch_val).grid(row=2, column=3)
        
        # reverse button.
        reverse_button = Button(self, text="Reverse", fg="black", bg=self.inactive_color, padx=10, pady=5, command=lambda: self.toggle_btn_click(reverse_button))
        reverse_button.grid(row=3, column=1, columnspan=5, sticky=W)
        
        # overlap.
        overlap_button = Button(self, text="Overlap", fg="Black", bg=self.inactive_color, padx=10, pady=5, command=lambda: self.toggle_btn_click(overlap_button))
        overlap_button.grid(row=4, column=1, sticky=W) 
        overlap_entry = Entry(self, width=30)
        overlap_entry.grid(row=4, column=2)
        
        # sequential.
        seq_button = Button(self, text="Sequential", fg="Black", bg=self.inactive_color, padx=10, pady=5, command=lambda: self.toggle_btn_click(seq_button))
        seq_button.grid(row=5, column=1, sticky=W) 
        seq_entry = Entry(self, width=30)
        seq_entry.grid(row=5, column=2)
        
        # rand insert.
        rand_button = Button(self, text="Random Insert", fg="Black", bg=self.inactive_color, padx=10, pady=5, command=lambda: self.toggle_btn_click(rand_button))
        rand_button.grid(row=6, column=1, sticky=W) 
        rand_entry = Entry(self, width=30)
        rand_entry.grid(row=6, column=2)
        
        # play button.
        button = Button(self, text="Play", fg="White", bg=self.default_button_color, padx=30, pady=5)
